    ...
    new AndroidCode(Vector("{\n" +
      ...
      paths.foldLeft(Vector.empty[String]) { (sofar, cmd) =>
        sofar ++ (cmd match {
          ...
          case MoveTo(coords) => foldCmd[Coords](coords, {
            case (x, y) => setCoords(x, y); s"path.moveTo($x, $y)"
          })
          case MoveToRel(coords) => foldCmd[Coords](coords, {
            case (x, y) => changeCoords(x, y); s"path.rMoveTo($x, $y)"
          })
          case LineTo(coords) => foldCmd[Coords](coords, {
            case (x, y) => setCoords(x, y); s"path.lineTo($x, $y)"
          })
          case LineToRel(coords) => foldCmd[Coords](coords, {
            case (x, y) => changeCoords(x, y); s"path.rLineTo($x, $y)"
          })
          ...
          case Cubic(args) => foldCmd[(Coords, Coords, Coords)](args, {
            case ((x1, y1), (x2, y2), (x, y)) =>
              setCoords(x, y)
              s"path.cubicTo($x1, $y1, $x2, $y2, $x, $y)"
          })
          case CubicRel(args) => foldCmd[(Coords, Coords, Coords)](args, {
            case ((x1, y1), (x2, y2), (x, y)) =>
              changeCoords(x, y)
              s"path.rCubicTo($x1, $y1, $x2, $y2, $x, $y)"
          })
          case SmoothCubic(args) => ??? // TODO
          case SmoothCubicRel(args) => ??? // TODO
          case Quad(args) => foldCmd[(Coords, Coords)](args, {
            case ((x1, y1), (x, y)) =>
              setCoords(x, y)
              s"path.quadTo($x1, $y1, $x, $y)"
          })
          case QuadRel(args) => foldCmd[(Coords, Coords)](args, {
            case ((x1, y1), (x, y)) =>
              changeCoords(x, y)
              s"path.rQuadTo($x1, $y1, $x, $y)"
          })
