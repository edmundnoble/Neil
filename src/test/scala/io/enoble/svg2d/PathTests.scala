...
import io.enoble.svg2d.render.{InitialPathRenderer, InitialRenderer}
...
class PathTests extends FunSuite {
  ...
  val parsers = new Path.Parsers(InitialPathRenderer)
  import parsers._
  ...
  test("A simple path should be parsable") {
    ...
    assert(parsed == Success(Vector(MoveToRel(Vector((25.000985, 1015.208))), CubicRel(Vector(((-0.23015, 0.0), (-0.36032, 0.05), (-0.55274, 0.2422)))),
      LineToRel(Vector((-1.6914, -1.6914))), CubicRel(Vector(((-0.1936, -0.1936), (-0.32064, -0.2422), (-0.55078, -0.2422)))), ClosePath()), simplePath.length))
  }

  test("A quadratic should be parsable") {
    val simpleQuad = "q 0,0 0,0"
    val parsed = quad.parse(simpleQuad)
    assert(parsed == Success(Vector(QuadRel(Vector(((0.0, 0.0), (0.0, 0.0))))), simpleQuad.length))
  }

  test("An elliptic should be parsable") {
    val simpleElliptic = "a 0,0 0 0,0 0,0"
    val parsed = ellipticalArc.parse(simpleElliptic)
    assert(parsed == Success(Vector(EllipticRel(Vector(EllipticParam((0.0, 0.0), 0.0, largeArc = false, sweep = false, (0.0, 0.0))))), simpleElliptic.length))
  }
  ...
  test("A moveto should be parsable as a full path") {
    ...
    assert(parsed == Success(Vector(MoveToRel(Vector((157.86809,496.83258), (0.0,-32.796), (-5.16279,0.0), (0.0,32.796), (5.16279,0.0)))), simpleMoveTo.length))
  }
  ...
  test("This path from camera_video_record.svg should be parsable") {
    ...
    val Success(_, _) = path.parse(testPath)
  }

  test("Paths with invalid elements should not be parsable") {
    val testPath = "x 0,1"
    val Failure(_, _) = path.parse(testPath)
  }
}