...
class PathTests extends FunSuite {
  ...
  test("A simple path should be parsable") {
    ...
    assert(parsed == Success(Path(ArrayBuffer(MoveToRel(ArrayBuffer((25.000985, 1015.208))), CubicRel(ArrayBuffer(((-0.23015, 0.0), (-0.36032, 0.05), (-0.55274, 0.2422)))),
  ...
  test("A quadratic should be parsable") {
    ...
    assert(parsed == Success(QuadRel(ArrayBuffer(((0.0, 0.0), (0.0, 0.0)))), simpleQuad.length))
  }

  test("An elliptic should be parsable") {
    val simpleElliptic = "a 0,0 0 0,0 0,0"
    val parsed = ellipticalArc.parse(simpleElliptic)
    assert(parsed == Success(EllipticRel(ArrayBuffer(EllipticParam((0.0, 0.0), 0.0, largeArc = false, sweep = false, (0.0, 0.0)))), simpleElliptic.length))
  }
  ...
  test("A moveto should be parsable as a full path") {
    ...
    assert(parsed == Success(Path(ArrayBuffer(MoveToRel(ArrayBuffer((157.86809,496.83258), (0.0,-32.796), (-5.16279,0.0), (0.0,32.796), (5.16279,0.0))))), simpleMoveTo.length))
  }
